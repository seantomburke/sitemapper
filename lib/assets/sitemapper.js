...
function () {
  ...
  _createClass(Sitemapper, [{
    ...
    value: function parse() {
      ...
      var requestOptions = _objectSpread({}, this.settings, {
        method: 'GET',
        url: url,
        timeout: this.timeout,
      ...
      return new Promise(function (resolve) {
        (0, _axios["default"])(requestOptions).then(function (response) {
          if (!response || response.status !== 200) {
          ...
          } else if (_this2.settings.responseType === 'stream') {
            ...
            writeable.on("end", function () {
              (0, _xml2jsEs6Promise["default"])(Buffer.concat(chunks));
            });
          }
          ...
          var message = _axios["default"].isCancel(error) ? 'request timed out' : error.stack;
          resolve({
            error: message,
            data: {}
          });
        ...
        _this2.initializeTimeout(url, resolve);
      });
    }
    /**
    ...
    value: function initializeTimeout(url, callback) {
      var _this3 = this;
    ...
    value: function crawl(url) {
      ...
      return new Promise(function (resolve) {
        _this4.parse(url).then(function (_ref) {
          ...
          if (error) {
            // Fail silently
            if (_this4.debug) console.debug('[Debug]:', error); // eslint-disable-line no-console
            else if (_this4.progress) process.stderr.write('x');
            return resolve([]);
          } else if (data && data.urlset && data.urlset.url) {
            var sites = data.urlset.url.map(function (site) {
              return site.loc && site.loc[0];
            });
            if (_this4.debug) console.debug('[Debug]:', sites.length, ' sites retrieved'); // eslint-disable-line no-console
            else if (_this4.progress) process.stdout.write('.');
            ...
            if (_this4.debug) console.debug('[Debug]: Diving deeper into', sitemap.length, 'more sitemaps'); // eslint-disable-line no-console
            else if (_this4.progress) process.stdout.write('↓'); // Make sure all the promises resolve then filter and reduce the array
