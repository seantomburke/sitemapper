...
class Sitemapper {
  ...
  fetch() {
    ...
    return new Promise(resolve => this.crawl(url).then(sites => resolve({
      url,
      sites
    })));
  }
  ...
  initializeTimeout(url, requester, callback) {
    ...
    this.timeoutTable[url] = setTimeout(() => {
      ...
      callback({
        error: "request timed out after ".concat(this.timeout, " milliseconds"),
        data: {}
      });
    }, this.timeout);
  }
    ...
    return new Promise(resolve => {
      this.parse(url).then((_ref) => {
        ...
        if (error) {
          ...
          var sites = data.urlset.url.map(site => site.loc && site.loc[0]);
          ...
          var promiseArray = sitemap.map(site => this.crawl(site)); // Make sure all the promises resolve then filter and reduce the array

          return Promise.all(promiseArray).then(results => {
            var sites = results.filter(result => !result.error).reduce((prev, curr) => prev.concat(curr), []);
            return resolve(sites);
          });
        }

        if (this.debug) {
          console.error("Unknown state during \"crawl(".concat(url, ")\""));
        } // Fail silently
